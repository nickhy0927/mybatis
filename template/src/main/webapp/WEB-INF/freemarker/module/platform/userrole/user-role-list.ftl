...
<@htmlHead>
    <script type="text/javascript">
        function create() {
            $.openWindow('创建模板', '80%', '80%', "${basePath}/userrole/user-role-create.do");
        }

        $(document).ready(function () {
            $("#dataGridList").dataGrid({
                url: ctx + '/userrole/user-role-list.json',
                title: 'UserRole管理列表',
                method: 'POST',
                checkbox: true,
                pageSize: 6,
                ...
                columns: [
					...
					{field: 'status', className: 'text-l', description: '有效状态1 有效 0 无效 ', sort: true},
					{field: 'userId', className: 'text-l', description: '用户ID ', sort: true},
					{field: 'roleId', className: 'text-l', description: '角色ID', sort: true},
					{field: 'id', className: 'text-l', description: '', sort: true},
					{field: 'createTime', className: 'text-l', description: '', sort: true},
					{field: 'updateTime', className: 'text-l', description: '', sort: true},
...
<@htmlBody>
	<nav class="breadcrumb">
        ...
        <span class="c-gray en">&gt;</span> UserRole管理
        <span class="c-gray en">&gt;</span> UserRole列表
