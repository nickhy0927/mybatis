...
<@htmlHead>
    <script type="text/javascript">
        function create() {
            $.openWindow('创建模板', '80%', '80%', "${basePath}/role/role-create.do");
        }

        $(document).ready(function () {
            $("#dataGridList").dataGrid({
                url: ctx + '/role/role-list.json',
                title: 'Role管理列表',
                method: 'POST',
                checkbox: true,
                pageSize: 6,
                ...
                columns: [
					...
					{field: 'roleId', className: 'text-l', description: '上级角色 ', sort: true},
					{field: 'remark', className: 'text-l', description: '信息备注 ', sort: true},
					{field: 'frozen', className: 'text-l', description: '是否冻结 1 冻结 0 未冻结', sort: true},
