...
[@htmlHead]
    <script type="text/javascript">
        function create() {
            $.openWindow('创建模板', '80%', '80%', "${basePath}/deploy/poster/poster-create.do");
        }
        
        function edit(id) {
            $.openWindow('修改模板', '80%', '80%', "${basePath}/deploy/poster/poster-edit/"+ id +".do");
        }
        
        function del(id, single) {
            $.datadel({
            	url: "${basePath}/deploy/poster/poster-delete/"+ id +".do",
                type: "post",
                data: {id: id},
		...
		function initData() {
			$("#dataGridList").dataGrid({
                url: ctx + '/deploy/poster/poster-list.json',
                title: 'Poster管理列表',
                method: 'POST',
                checkbox: true,
                method: 'POST',
...
[@htmlBody]
	<nav class="breadcrumb">
        ...
        <span class="c-gray en">&gt;</span> Poster管理
        <span class="c-gray en">&gt;</span> Poster列表
