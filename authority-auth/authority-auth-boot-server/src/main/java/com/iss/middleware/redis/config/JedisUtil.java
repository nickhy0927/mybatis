package com.iss.middleware.redis.config;
...
public class JedisUtil<T> {
	...
	public static List<Object> getObjectList(String key, Class<?> clazz) {
		...
		try {
			...
			if (jedis.exists(JSON.toJSONBytes(key))) {
				...
				for (byte[] bs : list) {
					value.add(JSON.parseObject((JSON.parse(bs)).toString(), clazz));
				}
	...
	public static Set<Object> getObjectSet(String key) {
		...
		try {
			jedis = jedisPool.getResource();
			if (jedis.exists(JSON.toJSONBytes(key))) {
				value = Sets.newConcurrentHashSet();
	...
	public static long setObjectSet(String key, Set<Object> value, int cacheSeconds) {
		...
		try {
			...
			Set<byte[]> set = Sets.newConcurrentHashSet();
			for (Object o : value) {
