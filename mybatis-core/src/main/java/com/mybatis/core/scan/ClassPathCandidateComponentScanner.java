...
public class ClassPathCandidateComponentScanner {
	...
	private static Logger logger = LoggerFactory.getLogger(ClassPathCandidateComponentScanner.class);
     ...
     * @throws InstantiationException 
     */
    private static void findClassesByFile(String pkgName, String pkgPath, Set<Class<?>> classes) throws InstantiationException, IllegalAccessException {
    ...
    private static void findClassesByJar(String pkgName, JarFile jar, Set<Class<?>> classes) {
        ...
        while (entry.hasMoreElements()) {
            ...
            if (claze != null) {
            	Table table = claze.getDeclaredAnnotation(Table.class);
            	if (table != null) {
            		classes.add(claze);
				}
            }
